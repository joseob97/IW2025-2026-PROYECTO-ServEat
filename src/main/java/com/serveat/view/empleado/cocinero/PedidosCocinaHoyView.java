...
public class PedidosCocinaHoyView extends VerticalLayout {
    ...
    public PedidosCocinaHoyView(CocineroService cocineroService,
        ...
        H3 titulo = new H3("Pedidos de hoy (Cocina)");
        titulo.getStyle().set("margin", "0");

        Button irHistorico = new Button("📚 Ver histórico");
        irHistorico.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
        irHistorico.getStyle().set("font-weight", "700");
        irHistorico.addClickListener(e -> getUI().ifPresent(ui -> ui.navigate("empleado/cocinero/historico")));

        HorizontalLayout top = new HorizontalLayout(titulo, irHistorico);
        top.setWidthFull();
    ...
    private void configurarGrid() {
        ...
        grid.addComponentColumn(p -> {
            Button ver = new Button("✏️ Actualizar");
    ...
    private void cargarPagina(int index) {
        try {
            ...
            Page<Pedido> page = cocineroService.buscarPedidosCocinaHoy(d, h, estado, mesa, pageable);
            ...
            infoPagina.setText("");
            Notification.show("Error cargando pedidos: " + ex.getMessage(), 4500, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
        }
    }
