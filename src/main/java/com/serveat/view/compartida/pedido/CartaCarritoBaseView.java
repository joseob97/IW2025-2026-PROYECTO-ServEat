...
/**
 * Base común: Carta + Carrito + Personalización.
 * No sabe nada de "cliente/camarero", solo ofrece hooks.
 * Lo usan CrearPedidoCartaView (Cliente) y IniciarPedidoView (Camarero)
 */
public abstract class CartaCarritoBaseView extends VerticalLayout {

    protected final transient PedidoCarritoService carritoService;
    protected final transient PedidoCalculoService calculoService;
    ...
    protected final ComboBox<String> filtroCategoria = new ComboBox<>("Categoría");
    ...
    protected final Button btnEditarCarrito = new Button("Editar carrito");

    protected CartaCarritoBaseView(PedidoCarritoService carritoService,
    ...
    /**
     * Hook: controla si se puede añadir/personalizar producto (cliente siempre true,
     * camarero depende de si hay pedido creado).
     */
    protected abstract boolean puedeInteractuarConCarta();

    /**
     * Hook: controla si se muestra/habilita personalización (cliente depende feature, camarero true).
     */
    protected abstract boolean personalizacionHabilitada();

    /**
     * Hook: se ejecuta tras refrescar el carrito (para habilitar botones, etc).
     */
    protected abstract void onCarritoActualizado();

    /**
     * Construye el layout Carta+Carrito y permite inyectar una tarjeta extra en la derecha (detalles/confirmar).
     */
    protected final Component construirCartaYCarrito(Component cardDerechaAbajo) {
        configurarFiltros();
        ...
        btnEditarCarrito.addClickListener(e -> {
            ...
            btnEditarCarrito.setText(editarCarrito ? "Listo" : "Editar carrito");
        ...
        derecha.add(cardCarrito);
        if (cardDerechaAbajo != null) derecha.add(cardDerechaAbajo);
    ...
    protected final void cargarProductos() {
    ...
    private Component crearCardProducto(Producto p) {
        ...
        Button add = new Button("Añadir", e -> {
            if (!puedeInteractuarConCarta()) return;

            try {
                int cantidad = (qty.getValue() != null) ? qty.getValue() : 1;
                carrito = carritoService.agregarProducto(carrito, p, cantidad);
                refrescarCarrito();
                Notification.show("Añadido: " + safe(p.getNombre() != null ? p.getNombre() : p.getCodigo()),
        ...
        boolean tieneIng = personalizacionHabilitada()
                && p.getCodigo() != null
                && cacheTieneIngredientes.getOrDefault(p.getCodigo(), false);
        ...
        personalizar.setEnabled(puedeInteractuarConCarta() && tieneIng);
        if (personalizacionHabilitada() && !tieneIng) {
        ...
        personalizar.addClickListener(e -> {
            if (!puedeInteractuarConCarta()) return;
            if (!personalizacionHabilitada()) return;
            if (!tieneIng) return;
    ...
    private void abrirDialogoPersonalizar(String codigoProducto, IntegerField qtyField) {
        ...
        try {
            ...
            List<ProductoIngrediente> recetaRaw =
                    (producto.getIngredientes() == null) ? List.of() : producto.getIngredientes();
            ...
            dialog.setHeaderTitle("Personalizar: " + safe(producto.getNombre() != null ? producto.getNombre() : producto.getCodigo()));
            ...
            if (!porDefecto.isEmpty()) {
                ...
                for (ProductoIngrediente pi : porDefecto) {
                    formDef.add(crearFilaPersonalizar(pi, quitarChecks, extras));
                }
            ...
            if (!noPorDefecto.isEmpty()) {
                ...
                for (ProductoIngrediente pi : noPorDefecto) {
                    formNoDef.add(crearFilaPersonalizar(pi, quitarChecks, extras));
                }
    ...
    private Component crearFilaPersonalizar(ProductoIngrediente pi,
    ...
    private Component accionesCarritoPorFila(LineaPedido lp) {
        ...
        Button trash = new Button("Eliminar", e -> eliminarLinea(lp));
    ...
    protected final void refrescarCarrito() {
        ...
        BigDecimal totalCalc = items.isEmpty()
                ? BigDecimal.ZERO
                : calculoService.calcularTotalPedido(carrito);

        total.setText("Total: " + totalCalc + " €");
        onCarritoActualizado();
    }

    protected final void setCartaEnabled(boolean enabled) {
        filtroCategoria.setEnabled(enabled);
        buscador.setEnabled(enabled);
        btnEditarCarrito.setEnabled(enabled);
        gridCarrito.setEnabled(enabled);
        cargarProductos();
    }
    ...
    protected int valueOrZero(Integer v) {
