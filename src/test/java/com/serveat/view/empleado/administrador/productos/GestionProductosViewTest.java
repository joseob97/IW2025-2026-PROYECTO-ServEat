...
class GestionProductosViewTest {
    ...
    void abrir_dialogo_nuevo_no_revienta_y_carga_listas_de_selector() throws Exception {
        ...
        // Dialog no cuelga del árbol de componentes del view, no se puede contar con getChildren()
        // Se valida: no lanza excepción y se han consultado los servicios para poblar selects
        assertDoesNotThrow(() ->
    ...
    private static MultiSelectComboBox<?> findMultiSelectComboBoxByLabel(Component root, String label) {
        for (Component c : flatten(root)) {
            if (c instanceof MultiSelectComboBox<?> cb && label.equals(cb.getLabel())) {
                return cb;
            }
        }
        return null;
